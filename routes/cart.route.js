...
/*
 @route POST /api/cart/:id/add
 @desc find cart for current user and make modifications
 @access private
 */
	...
	try {
		...
		if (checkedCart) {
			...
			if (cart.items) {
				...
				//find index of item in cart if it exists
			...
			if (savedCart) {
				...
				// need to populate to fill it with the info of items in the cart, or not the cart just contains ids of the items
...
/*
 ...
 @desc find cart for current user and shift cart to order
 @access private
 */
...
/*
 ...
 @desc allow the user to make payment via stripe
 @access private
 */
